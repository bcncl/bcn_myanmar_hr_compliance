...
class BCNSalarySlip(SalarySlip):
    ...
    def check_draft_status_slip(self):
        drafting_slip = frappe.db.get_list(
            "Salary Slip",
            filters={
                "docstatus": 0,
                "name": ["!=", self.name],
            ...
            or_filters={
                "posting_date": [
                    "between",
                    [self.payroll_period.start_date, self.payroll_period.end_date],
                ],
            },
            pluck="name",
        )
    ...
    def compute_income_tax_breakup(self):
        ...
        if hasattr(self, "tax_slab"):
            self.custom_bcn_myanmar_pit_applied = (
                self.tax_slab.custom_bcn_is_myanmar_pit_compliance
            )
        ...
        if self.custom_bcn_myanmar_pit_applied:
            if self.total_earnings > self.standard_tax_exemption_amount:
                ...
                self.standard_tax_exemption_amount = (
                    basic_relief if basic_relief < 10000000 else 10000000
                )
    ...
    def get_total_exemption_amount(self):
        ...
        if self.tax_slab.standard_tax_exemption_amount:
            if (
                self.custom_bcn_myanmar_pit_applied
                and self.total_earnings
                and self.standard_tax_exemption_amount
            ):
                if self.total_earnings > self.standard_tax_exemption_amount:
                    basic_relief = self.total_earnings * 0.2
                    self.standard_tax_exemption_amount = (
    ...
    def set_net_pay(self):
        ...
        self.custom_bcn_total_contribution = self.get_component_totals(
            "custom_bcn_contributions"
        )
        self.custom_bcn_base_total_contribution = flt(
            flt(self.custom_bcn_total_contribution) * flt(self.exchange_rate),
            self.precision("custom_bcn_base_total_contribution"),
        )
    ...
    def get_contribution_additional_salaries(
    ...
    def add_employer_contributions(self):
        ...
        for contribution in self.custom_bcn_contributions:
            self.current_month_structured_contribution += flt(
                contribution.amount, contribution.precision("amount")
            )

            if contribution.is_one_time_contribution == 1:
                self.one_time_contribution += flt(
                    contribution.amount, contribution.precision("amount")
                )

        self.future_structured_contributions = 0.0
        self.future_structured_contributions = (
            self.current_month_structured_contribution - self.one_time_contribution
        ) * (ceil(self.remaining_sub_periods) - 1)

        self.custom_bcn_contributed_amount_till_date = (
            self.previous_contributions + self.current_month_structured_contribution
        )
        self.custom_bcn_current_month_contribution = (
    ...
    def get_opening_contributed_amount(self):
        return (
            self.get_opening_for(
                "custom_bcn_contributed_amount_till_date",
    ...
    def compute_annual_deductions_before_tax_calculation(self):
        annual_deductions_before_tax_calculation = (
            super().compute_annual_deductions_before_tax_calculation()
        )
    ...
    def get_taxable_earnings_for_prev_period(
        ...
        (
            taxable_earnings,
            exempted_amount,
        ) = super().get_taxable_earnings_for_prev_period(
    ...
    def get_opening_exempted_before_tax_calculation(self):
        if self.payroll_period:
            return (
                self.get_opening_for(
                    "custom_bcn_exempted_from_income_tax_till_date",
                    self.payroll_period.start_date,
                    self.end_date,
                )
                or 0
    ...
    def get_opening_for(self, field_to_select, start_date, end_date):
        ...
        if not hasattr(self, "_opening_salary_structure_assignment"):
            ...
            self._opening_salary_structure_assignment = (
                opening_salary_structure_assignment[0]
                if opening_salary_structure_assignment
                else frappe._dict()
            )
