...
def execute(filters=None):
    ...
    for contribution in contributions:
        ...
        employee_salary = {"employer_ssc": 0.0, "employer_injury": 0.0}
        ...
        if contribution_map:
            ...
            if contribution_map.salary_component in ["SSC 2% (ER)", "SSC 2.5% (ER)"]:
                employee_salary.setdefault(
                    "employer_ssc", contribution_map.amount or 0.0
                )

            if contribution_map.salary_component == "SSC 1% (ER)":
                employee_salary.setdefault(
                    "employer_injury", contribution_map.amount or 0.0
                )
    ...
    for slip in salaries:
        ...
        employee_salary.setdefault(
            "employee_name",
            employee.custom_bcn_employee_name_mm or employee.employee_name,
        )
        ...
        employee_salary.setdefault(
            "payment", 300000 if slip.gross_pay > 300000 else slip.gross_pay
        )
        ...
        total_employer_ssc = (
            slip.employer_ssc + slip.employer_injury if slip.employer_injury else 0.0
        )
...
def get_salary_slip(filters):
    ...
    salaries = frappe.get_list(
        ...
        fields=["name", "employee", "posting_date", "gross_pay"],
        order_by="employee",
    )
...
def get_salary_deductions_detail(salary_name_list):
    myanmar_compliance_component_list = frappe.get_list(
        "Salary Component",
        filters={"custom_bcn_is_myanmar_ssc": 1, "type": "Deduction"},
        pluck="name",
    )
...
def get_contribution_detail(salary_name_list):
    ...
    contribution_detail = frappe.db.get_all(
        "BCN Contribution Detail",
        filters={"parent": ["In", salary_name_list]},
        fields=["name", "parent", "salary_component", "amount"],
    )
    if contribution_detail:
        contribution_detail_map = {
            contribution.name: contribution for contribution in contribution_detail
        }
